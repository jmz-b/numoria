...
int caveGetTileColor(Coord_t const &coord) {
    ...
    if (tile.treasure_id != 0) {
        int category_id = game.treasure.list[tile.treasure_id].category_id;
        int indexx = game.treasure.list[tile.treasure_id].sub_category_id & (ITEM_SINGLE_STACK_MIN - 1);
        int color = game_objects[game.treasure.list[tile.treasure_id].id].color;

        switch (category_id) {
            case TV_AMULET:
                return amulet_colors[indexx];
            case TV_RING:
                return rock_colors[indexx];
            case TV_STAFF:
                return wood_colors[indexx];
            case TV_WAND:
                return metal_colors[indexx];
            case TV_POTION1:
            case TV_POTION2:
                return potion_colors[indexx];
            case TV_FOOD:
                if ((indexx) < MAX_MUSHROOMS) {
                    return mushroom_colors[indexx];
                }
                return color;
            case TV_GOLD:
                if (tile.feature_id >= MIN_CAVE_WALL) {
                    return config::options::draw_solid_walls ? color + (MAX_COLORS - 1) : color;
                }
                return color;
            case TV_SECRET_DOOR:
                return config::options::draw_solid_walls ? Color_Wall + (MAX_COLORS - 1) : Color_Wall;
            default:
                return color;
        }
    }
        ...
        if (!config::options::highlight_seams) {
            ...
            switch(tile.feature_id) {
                case TILE_MAGMA_WALL:
                    return config::options::draw_solid_walls ? Color_Magma + (MAX_COLORS - 1) : Color_Magma;
                case TILE_QUARTZ_WALL:
                    return config::options::draw_solid_walls ? Color_Quartz + (MAX_COLORS - 1) : Color_Quartz;
                default:
                    return config::options::draw_solid_walls ? Color_Wall + (MAX_COLORS - 1) : Color_Wall;
            }
        }
