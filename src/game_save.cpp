...
static bool svWrite() {
    ...
    if (config::options::auto_haggle) { l |= 0x2000; }
    if (config::options::full_monster_recall) { l |= 0x4000; }
    if (config::options::auto_open_door) { l |= 0x8000; }
...
bool loadGame(bool &generate) {
    ...
    if (dg.game_turn >= 0) {
        ...
        config::options::auto_open_door = (l & 0x8000) != 0;
